...
def test_train_creates_production_bundle(sample_df, workspace_tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("ML_DPE_regression.src.train.METRICS_DIR", workspace_tmp_path)
    monkeypatch.setattr(
        "ML_DPE_regression.src.train.get_production_model_path",
        lambda: workspace_tmp_path / "modele.pkl",
    )
    monkeypatch.setattr(
        "ML_DPE_regression.src.train.get_latest_model_info_path",
